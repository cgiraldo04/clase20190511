...
#    # 20 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    ...
    F2 = tf.contrib.layers.flatten(Z5)
...
def model(X_train, Y_train, X_test, Y_test, learning_rate = 0.009, num_epochs = 30, minibatch_size = 16, print_cost = True):
    """
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % 10 == 0:
               save_path = saver.save(sess, "D:\\DeepLearning\\04 Redes Neuronales Convolucionales\\RedesConvolucionales_MNIST\\model_softmax.ckpt")
               print("Model saved in path: %s" % save_path)
               print("iteracion: %s" % str(epoch))
        ...
        print(accuracy)
        train_accuracy = accuracy.eval({X: X_train[0:10000,...], Y: Y_train[0:10000,...]})
