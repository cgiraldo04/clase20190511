...
def initialize_parameters():
    ...
    W1 = tf.get_variable("W1", [3, 3, 3, 4], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
    W2 = tf.get_variable("W2", [3, 3, 4, 8], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
    W3 = tf.get_variable("W2", [3, 3, 8, 16], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
    W4 = tf.get_variable("W2", [3, 3, 16, 32], initializer = tf.contrib.layers.xavier_initializer(seed = 0))
...
#    # 20 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    
    Z5 = tf.contrib.layers.fully_connected(F1, 20, None)
    
    # FLATTEN2
    
    F2 = tf.contrib.layers.flatten(Z3)
    
#    # FULLY-CONNECTED without non-linear activation function (not not call softmax).
#    # 10 neurons in output layer. Hint: one of the arguments should be "activation_fn=None" 
    
    Z6 = tf.contrib.layers.fully_connected(F2, 10, None)
...
def model_predict(data_test, y_label, learning_rate = 0.009, num_epochs = 100, minibatch_size = 10, print_cost = True):
    ...
    with tf.Session() as sess:
        ...
        saver.restore(sess, "D:\\DeepLearning\\04 Redes Neuronales Convolucionales\\RedesConvolucionales_MNIST\\model_softmax.ckpt")
...
img = cv2.imread("D:\\DeepLearning\\04 Redes Neuronales Convolucionales\\RedesConvolucionales_MNIST\\cero.png")
