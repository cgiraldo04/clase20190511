...
def inicializar_Parametros(n_x, n_h, n_y): 
    ...
    parametros --  Diccionario de Python que contiene a:
                    ...
                    llevar todos los pesos de manera aleatoria, n_x parametros capa de entrada, n_h capa oculta, n_Y capa de salida
    """
       
    np.random.seed(1)
    
    W1 = np.random.randn(n_x, n_h)*0.01 # se multiclipa por 0.01 para darle pesos pequeños a la red con la intencion de no saturar la red
    b2 = np.zeros((n_h, 1)) #
    W2 = np.random.randn(n_h, n_y)*0.01 # np.random.randn funcion que asigna pesos aleatoriamente
    b3 = np.zeros((n_y, 1))#
    
    assert(W1.shape == (n_x, n_h)) # verificar tamaños
    assert(b2.shape == (n_h, 1))
    assert(W2.shape == (n_h, n_y))
    assert(b3.shape == (n_y, 1))
    
    parameters = {"W1": W1, # diccionario o base de datos integradas en una sola variable, lista de estructuras
                  "b2": b2,
                  "W2": W2,
                  "b3": b3}
...
#
def inicializar_parametros_profundos(layer_dims): #[1,2,3,4] ejemplo de arquitedtura definiendo numero de neuronas
    """
    Argumenots:
...
def compute_cost(AL, YS, costFuntion):
    ...
    if costFuntion == "LG":     # entreopia logistica
    
        cost = -np.sum(np.multiply(np.log(AL),YS) + np.multiply(np.log(1-AL),(1-YS)))/m
        
    elif costFuntion == "MSE":   #Error cuadrat
...
def linear_backward(dZ, cache): # dZ claculo del error cuadratico con respecto a Z dZ=dE/dZ
    """
...
#
def update_parameters(parameters, grads, learning_rate): #regla del descenso del gradiente
    """
